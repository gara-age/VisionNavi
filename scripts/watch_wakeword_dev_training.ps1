...
$entries = @(
  @{
    ...
    ConfigPattern = "ko_nabiya_balanced"
    OutputDir = "ko_nabiya_dev"
  },
  @{
    Id = "ja_nee_navi_dev"
    Label = ([string]([char]0x306D) + [char]0x3048 + [char]0x3001 + [char]0x30CA + [char]0x30D3)
    ConfigPattern = "ja_nee_navi_balanced"
    OutputDir = "ja_nee_navi_dev"
  },
  @{
    Id = "ko_hey_nabi_dev"
    Label = ([string]([char]0xD5E4) + [char]0xC774 + " " + [char]0xB098 + [char]0xBE44)
    ConfigPattern = "ko_hey_nabi_balanced"
    OutputDir = "ko_hey_nabi_dev"
  },
  @{
    Id = "ja_navisan_dev"
    Label = ([string]([char]0x30CA) + [char]0x30D3 + [char]0x3055 + [char]0x3093)
    ConfigPattern = "ja_navisan_balanced"
    OutputDir = "ja_navisan_dev"
  }
)

$queueStdoutLog = Join-Path $logRoot "wakeword_training_dev_queue.log"
$queueStderrLog = Join-Path $logRoot "wakeword_training_dev_queue.error.log"
...
function Get-QueueState {
  $queueProcess = @(Get-CimInstance Win32_Process |
      Where-Object {
        ...
        $_.CommandLine -like "*-Preset balanced*"
      } |
      Select-Object ProcessId, Name, CommandLine)
...
function Get-OutputSummary {
  ...
  if ($files.Count -gt 0) {
    ...
    $dirItem = Get-Item $DirectoryPath
    $latest = $dirItem.LastWriteTime
  }
...
function Write-Header {
  ...
  Write-Host "VisionNavi Wakeword Dev Monitor  $now" -ForegroundColor White
